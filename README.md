...
⚡ 1 Otimização de Performance com Cache (`.cache()`)
    Conceito Aplicado: O PySpark utiliza "Lazy Evaluation" (avaliação preguiçosa), o que significa que as transformações só são executadas quando uma ação é chamada.
    Solução: Aplicamos o método `.cache()` logo após o saneamento inicial dos dados (remoção de nulos e duplicados).

 Resultado: O DataFrame intermediário é persistido diretamente na memória RAM. Isso evita que o Spark precise reprocessar todo o grafo de execução (DAG) do zero a cada novo cruzamento ou exportação, economizando tempo e recursos computacionais.
 ...
 🧩 2. Modularização e Clean Code (Funções Reutilizáveis)
    ...
    Solução: Substituição de execuções sequenciais por funções puras e modulares (ex: `def limpar_dados_videos(df):`).
